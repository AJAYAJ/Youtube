...
import com.myworks.youtubeapp.menu.YouTubePlayerMenu
import com.myworks.youtubeapp.menu.defaultMenu.DefaultYouTubePlayerMenu
import com.myworks.youtubeapp.utils.FadeViewHelper
import com.myworks.youtubeapp.views.YouTubePlayerSeekBar
import com.myworks.youtubeapp.views.YouTubePlayerSeekBarListener
...
class DefaultPlayerUiController(
  ...
  private fun initClickListeners() {
    ...
    customActionLeft.setImageResource(R.drawable.ayp_ic_rewind_10_24dp)
    customActionLeft.setOnClickListener {
        val currentTime = youtubePlayerSeekBar.seekBar.progress.toFloat()
        youTubePlayer.seekTo(Math.max(0f, currentTime - 10f))
    }

    customActionRight.setImageResource(R.drawable.ayp_ic_forward_10_24dp)
    customActionRight.setOnClickListener {
        val currentTime = youtubePlayerSeekBar.seekBar.progress.toFloat()
        val duration = youtubePlayerSeekBar.seekBar.max.toFloat()
        youTubePlayer.seekTo(Math.min(duration, currentTime + 10f))
    }
